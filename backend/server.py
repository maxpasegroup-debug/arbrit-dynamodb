...
# Removed duplicate endpoint - consolidated into the secured version above
...
async def create_expense_claim(claim_data: ExpenseClaimCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Convert all floats to Decimals for DynamoDB compatibility
        claim_dict = convert_floats_to_decimals(claim_dict)
    ...
    try:
        ...
        scenarios = [
            ...
            },
            {
                ...
                "course_name": "NEBOSH International Diploma",
                "training_date": (datetime.now(timezone.utc) - timedelta(days=100)).strftime("%Y-%m-%d"),
                "participants_count": 8,
                "certificate_numbers": [f"NEBOSH-2024-{str(i).zfill(3)}" for i in range(1, 9)],
                "certificate_type": "International",
                "dispatch_status": "in_transit",
                "courier_service": "DHL Express",
                "tracking_number": "DHL-UAE-2024-56789",
                "dispatch_date": (datetime.now(timezone.utc) - timedelta(days=10)).strftime("%Y-%m-%d"),
                "expected_delivery_date": (datetime.now(timezone.utc) + timedelta(days=5)).strftime("%Y-%m-%d"),
                "actual_delivery_date": None,
                "delivery_note_photo": None,
                "recipient_name": None
            },
            ...
            },
            {
                ...
                "training_date": (datetime.now(timezone.utc) - timedelta(days=4)).strftime("%Y-%m-%d"),
                "participants_count": 8,
            ...
            },
            {
                ...
                "course_name": "IOSH Managing Safely",
                "training_date": (datetime.now(timezone.utc) - timedelta(days=45)).strftime("%Y-%m-%d"),
                "participants_count": 12,
                "certificate_numbers": [f"IOSH-2024-{str(i).zfill(3)}" for i in range(1, 13)],
                "certificate_type": "International",
            ...
            },
            {
                ...
                "training_date": (datetime.now(timezone.utc) - timedelta(days=5)).strftime("%Y-%m-%d"),
