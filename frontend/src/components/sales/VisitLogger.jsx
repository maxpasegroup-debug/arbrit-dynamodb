    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {visits.length === 0 ? (
          ...
          visits.map((visit) => (
            <Card key={visit.id} className="bg-slate-900 border-white/20 p-4">
        ...
        <DialogContent className="bg-[#1a2f4d] border-white/20 text-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input type="date" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} required className="bg-slate-900 border-white/20 text-white mt-1" />
            </div>
            <div>
              <Label className="text-gray-300">Company *</Label>
              <Input value={formData.company} onChange={(e) => setFormData({...formData, company: e.target.value})} required className="bg-slate-900 border-white/20 text-white mt-1" />
            </div>
            <div>
              <Label className="text-gray-300">Contact Person *</Label>
              <Input value={formData.contact_person} onChange={(e) => setFormData({...formData, contact_person: e.target.value})} required className="bg-slate-900 border-white/20 text-white mt-1" />
            </div>
            <div>
